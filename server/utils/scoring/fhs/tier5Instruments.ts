...
export async function computeTier5Instruments(
    ...
    try {
        ...
        if (pids.length) {
            ...
            const debtDueFact = debtDue18m
                ? extractPropertyFacts(values, debtDue18m)[0]
                : undefined;
            const cashFact = cashPid ? extractPropertyFacts(values, cashPid)[0] : undefined;
            const longDebtFact = longTermDebt
