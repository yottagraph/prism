...
export async function computeTier3Behavioral(event: H3Event, neid: string): Promise<FhsTierResult> {
    ...
    try {
        ...
        if (filingPid) {
            ...
            const latestFiling = filingFacts[0];
            const filingGapDays = recencyDays(
                latestFiling?.date ??
                    (typeof latestFiling?.value === 'string' ? latestFiling.value : undefined)
            );
            if (filingGapDays != null) {
                ...
                if (severity) {
                    ...
                    if (latestFiling?.ref) refs.push(latestFiling.ref);
                }
            }
        }
        ...
        const officerSignal = severityFromThresholds(departures90.length, [
        ...
        if (officerSignal) {
            signals.push({
                ...
                weight: 1.2,
                description: `${departures90.length} leadership departures in the last 90 days.`,
                evidence: [],
            });
        }
        metrics.push({ label: 'Departures (90d)', value: `${departures90.length}` });
        ...
        metrics.push({ label: 'Auditor changes (12m)', value: `${auditorChanges.length}` });
        if (auditorChanges.length >= 2) {
            signals.push({
                ...
                score: 85,
                weight: 1.1,
                description: `${auditorChanges.length} auditor changes in the past 12 months.`,
                evidence: [],
            });
        } else if (auditorChanges.length === 1) {
            signals.push({
                signalType: 'auditor_changes',
                tier: 3,
                severity: 'high',
